...
public class Meydan {
    ...
    private List<Asker> setTeams(List<Asker> Team){
        ...
        if(Team.isEmpty()){
            for(int i = 0; i < rnd_number; i++){
                Team.add(new Tegmen());
                Team.get(i).name = "tegmen" + (i+1);
            }
        }else{
            for(int i = 0; i < rnd_number; i++){
                Team.add(new Tegmen());
                Team.get(i+1).name = "tegmen" + (i+1);
            }
        }
    ...
    protected static void kill(Asker target){
        Alan[target.pos.x][target.pos.y] = null;
        if(target.team == 'A'){
            TeamA.remove(target);
        }else {
            TeamB.remove(target);
    ...
    protected static Asker checkAround(Asker asker){
        ...
        if(asker.getClass() == Er.class){
            p = new Point(asker.pos.x-1,asker.pos.y-1);
            n = 3;
            s = 1;
        } else if(asker.getClass() == Tegmen.class){
            p = new Point(asker.pos.x-2,asker.pos.y-2);
            n = 5;
            s = 2;
        }else {
            p = new Point(asker.pos.x-3,asker.pos.y-3);
            n = 7;
            s = 3;
        }
    ...
    private static void printTeams(){
        ...
        for(int i=0;i<7;i++){
            System.out.println(TeamA.get(i).name);
        }
        System.out.println();
        System.out.println("TeamB");
        for(int i=0;i<7;i++){

            System.out.println(TeamB.get(i).name);
        }
        System.out.println();
    }
